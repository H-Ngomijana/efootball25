...
const Header = ({fixed = false})=>{
    ...
    return <header className={` px-4 md:px-[10%] py-4 text-white flex justify-between items-center ${fixed? 'fixed w-full bg-transparent' : 'sticky bg-primary'} top-0 z-20`}>
        <Link to='/' className='text-fourth text-2xl md:text-3xl uppercase'>
            efootball
            <p className='text-sm'>By Ngomijana</p>
        </Link>
        <nav className='hidden md:flex items-center gap-4 '>
            ...
            <div className='hidden lg:flex items-center gap-1'>
                <a className='flex items-center justify-center rounded-full w-[40px] h-[40px] bg-[#181717]' href='https://github.com/H-Ngomijana' target='_blank'><GitHub/></a>
                <a className='flex items-center justify-center rounded-full w-[40px] h-[40px] bg-gradient-to-r from-yellow-400 via-pink-500 to-purple-600' href='https://instagram.com/ngomijana_' target='_blank'><Instagram/></a>
            </div>
        </nav>
